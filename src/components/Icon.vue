<template>
  <span :class="(button ? ' ' : 'icon-text ') + (containerClass ?? '')">
    <span v-if="$slots.defaultLeft || textLeft" :class="textClass">
      <slot name="text-left">{{ textLeft }}</slot>
    </span>
    <span :class="'icon ' + (iconClass ?? '')">
      <FontAwesomeLayers full-width :class="layerClass">
        <FontAwesomeIcon :icon="icon" :spin="spin" :rotation="rotate" :size="size" />
        <FontAwesomeLayersText v-if="counter != undefined" counter :value="counter" position="top-right" />
      </FontAwesomeLayers>
    </span>
    <span v-if="$slots.default || text" :class="textClass">
      <slot>{{ text }}</slot>
    </span>
  </span>
</template>

<script setup lang="ts">
import { FontAwesomeIcon, FontAwesomeLayers, FontAwesomeLayersText } from '@fortawesome/vue-fontawesome'
const props = defineProps<{
  icon: string,
  text?: string,
  textLeft?: string
  containerClass?: string
  iconClass?: string,
  spin?: boolean,
  button?: boolean,
  rotate?: 90 | 180 | 270 | "90" | "180" | "270" | undefined,
  size?: "2xs" | "xs" | "sm" | "lg" | "xl" | "2xl" | "1x" | "2x" | "3x" | "4x" | "5x" | "6x" | "7x" | "8x" | "9x" | "10x" | undefined,
  counter?: number,
  layerClass?: string,
  textClass?: string
}>()
</script>
